...
// Timer used for the PWM
#define LED_PWM_TIM TIM10

#define LED_SET   0b00000000000000000001000000000000
#define LED_RESET 0b00001000000000000000000000000000
...
int main(void)
{
    ...
    DBG("LED Port Address 0x%08lx (0x%08lx)\n", (uint32_t)&LED_GPIO_Port->BSRR, (uint32_t )led_bb_bit);
    ...
    uint32_t now = 0, last_tick = 0, last_change = 0, last_int_cnt = 0;
    ...
    while (1) {
        ...
        if (now - last_tick >= 1000) {
            ...
            DBG("Tick %lu (loop = %lu int = %lu) val = 0x%02x\n", now / 1000, loop_cnt, led_pwm_cnt - last_int_cnt, led_pwm_val);
